        ...
        <Tabs defaultValue="favorites" className="w-full">
          ...
          <TabsContent value="favorites">
            <div className="space-y-8">
              <div>
                ...
                {favoritesLoading ? (<div className="flex justify-center py-8">
                    <Loader2 className="w-8 h-8 animate-spin text-primary"/>
                  </div>) : movieFavorites.length === 0 ? (<p className="text-muted-foreground">No movie favorites yet</p>) : (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {movieFavorites.map((fav) => (<MovieCard key={fav.id} movie={{
              ...
              <div>
                ...
                {favoritesLoading ? (<div className="flex justify-center py-8">
                    <Loader2 className="w-8 h-8 animate-spin text-primary"/>
                  </div>) : bookFavorites.length === 0 ? (<p className="text-muted-foreground">No book favorites yet</p>) : (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {bookFavorites.map((fav) => (<BookCard key={fav.id} book={{
                    id: fav.item_id,
                    volumeInfo: {
                        title: fav.title,
          ...
          <TabsContent value="reviews">
            {reviewsLoading ? (<div className="flex justify-center py-8">
                <Loader2 className="w-8 h-8 animate-spin text-primary"/>
              </div>) : reviews.length === 0 ? (<p className="text-muted-foreground">No reviews yet</p>) : (<div className="grid gap-4">
