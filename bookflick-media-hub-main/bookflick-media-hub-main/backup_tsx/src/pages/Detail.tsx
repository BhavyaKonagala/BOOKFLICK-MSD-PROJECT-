...
const Detail = () => {
  ...
  const genre = isMovie ? item.genres?.map((g: any) => g.name).join(", ") : item.volumeInfo?.categories?.join(", ");
  const creator = isMovie ? item.credits?.crew?.find((c: any) => c.job === "Director")?.name : item.volumeInfo?.authors?.join(", ");
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          <div className="md:col-span-1">
            <img
              src={poster !== "N/A" && poster ? poster : "/placeholder.svg"}
              alt={title}
